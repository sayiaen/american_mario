...
import ch.hevs.gdx2d.hello.{Assets, Bullet, GameManager, LootBox, RPG, enemies}
import com.badlogic.gdx.graphics.Color
import ch.hevs.gdx2d.lib.GdxGraphics
import scala.collection.mutable.ListBuffer

// deathmanager is used to takecare of all damage and death conditions for entities(enemies, player and lootboxes)
...
//check status is to check the existing/deleting conditions of enemies lootboxes, rpg, bullets and player
        ...
        for (e <- enemieslist if !deadEnemies.contains(e)) {
          ...
          if (distance <= explosionRadius) {
            if (e.takeDamage(explosionDamage)) {
              deadEnemies += e
              GameManager.scoreCalculator(500)
            }
          }
        }
        ...
        for (lb <- lootBoxList if !usedLootBoxes.contains(lb)) {
          ...
          if (distance <= explosionRadius) {
            if (lb.takeDamage(explosionDamage)) {
              usedLootBoxes += lb
              Assets.lootBox_breakingau.setVolume(0,2f)
              Assets.lootBox_breakingau.play()
              GameManager.scoreCalculator(1000)
              GameManager.RPG += 3
...
//creation of enemies, bullets, lootboxes and grenades
    def draw(g: GdxGraphics, maxcamX: Float, player: Player): Unit = {
      g.setColor(Color.ORANGE)
      g.drawFilledRectangle(maxcamX + 960, Lava_level / 2, 1920, Lava_level, 0)
      ...
      g.drawString(maxcamX + 90, 1020, s"SCORE: ${GameManager.totalScore}", 200)
      g.drawString(maxcamX + 90, 980, s"GRENADE: ${GameManager.RPG}", 200)
      g.drawString(maxcamX + 90, 940, s"HEALTH: ${player.health}", 200)
