...
// minion class is used for the variables and methods of the minion enemies
...
// is used to calculate the movement and the collision logic(with platforms and player) for the minions
  override def update(dt: Float, platforms: List[Platform]): Unit = {
    ...
    // I used the gravity phsics to add more complex minioons but didn't add them
    vy += gravity * dt
    y += vy * dt
    minionRect.set(x,y,width, height)
    var onGround = false
    ...
    //Edge Detection is used to determine the borders which the minions can stroll
    if (onGround) {
...
// recttopolygon is a method used for collisions mainly fetched online
