...
//contains the variables for players used for dimensions and movement/phsics logic
...
//takedamage is used to calculate the remaining health after collision with enemies
  def takedamage(amount: Int): Unit = {
    if (invinnciblityttimer <= 0) {
      health -= amount
      invinnciblityttimer = damageCooldown // used to take one dame per collisison
...
// this colision logic is used to determine the platform and player collision so that we can walk and traval using them
  ...
  override def update(dt: Float, platforms: List[Platform], maxcamX: Float): Unit = {
  ...
  override def draw(g: GdxGraphics): Unit = {
    ...
    val region = new TextureRegion(activeplayerSkin)
