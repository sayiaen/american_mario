...
//boss class holds the boss physics and variables with different types of methods boss uses
 class Boss ( val StartX: Float, val StartY: Float, val bossArenaleftX: Float, val bossarenaWidth: Float) extends enemies(StartX, StartY, 80f, 80f, 200){
  ...
  // override damage method is used to calculate the damage inflicted to the boss to calculate its growth
  ...
  //this methd is used to calculate the growth of the boss till its reaches the max height and width
  def grow(): Unit = {
  ...
  //update method is to determine the direction, speed and vulnerability of the boss. and bordercollision logics
...
//taken by AI to calculate to borders of the enemies
...
//overriden draw method used to draw determine which boss skin through if conditions and case match
  override def draw(g: GdxGraphics): Unit = {
    GameManager.selectbossskin match{
